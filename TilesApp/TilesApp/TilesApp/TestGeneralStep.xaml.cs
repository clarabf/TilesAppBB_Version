...
namespace TilesApp
{
    public partial class TestGeneralStep : ContentPage
    {
        ...
        public TestGeneralStep(Tile t, int t_id, int m_steps, string wor, string url, int s_order)
        {
            InitializeComponent();
            tile = t;
            task_id = t_id;
        ...
        private async void GoToNextStep( object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Navigation.PopModalAsync(true);
                        Navigation.PushModalAsync(new TestGeneralStep(tile, new_task.id, max_steps, worker, next_step_url, next_step_order));
                    });
                }
            }
            catch (Exception ex)
